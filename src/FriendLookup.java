...
public class FriendLookup {
	...
	public FriendLookup() throws Exception{
		...
		try{ 
			...
			ResultSet rs=stmt.executeQuery("select * from UserFriend where UserName = '"+un+"'");  
			while(rs.next())  
				System.out.println(rs.getString("UserName")+" are friends with "+rs.getString("FriendName"));  
			con.close(); 
